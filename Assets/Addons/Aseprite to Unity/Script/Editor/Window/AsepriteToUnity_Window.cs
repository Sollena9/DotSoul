namespace AsepriteToUnity {
	using System.Collections;
	using System.Collections.Generic;
	using System.Text.RegularExpressions;
	using UnityEngine;
	using UnityEditor;
	using AsepriteToUnity.Saving;
	using AsepriteToUnity.Core;


	// === Main ===
	public partial class AsepriteToUnity_Window : MoenenEditorWindow {
		...
		// Global
		private const string TITLE = "Aseprite To Unity";
...
● Content inside () will be ignored when there's only one item.";
		...
		[MenuItem("Tools/Aseprite to Unity/Aseprite to Unity")]
		private static void OpenWindow () {
			try {
				var inspector = typeof(Editor).Assembly.GetType("UnityEditor.InspectorWindow");
				var window = inspector != null ?
					GetWindow<AsepriteToUnity_Window>("Aseprite To Unity", true, inspector) :
					GetWindow<AsepriteToUnity_Window>("Aseprite To Unity", true);
				window.minSize = new Vector2(275, 400);
				window.maxSize = new Vector2(600, 1000);
				// Icon
				var icon = GetImage("Icon.png");
				if (icon) {
					window.titleContent = new GUIContent("Aseprite To Unity", icon);
				}
		...
		private static void ToolsGUI () {
			LayoutF_Tool.Value = LayoutF(() => {
				LayoutH(() => {
					GUIRect(12, 1);
					LayoutV(() => {
						const int HEIGHT = 28;
						if (Button(GUIRect(0, HEIGHT), "Pixel Editor (Coming Soon...)", Color.grey, false)) {
		...
		private static void SettingGUI () {
			LayoutF_Setting.Value = LayoutF(() => {
				...
				LayoutF_Setting_NamingStrategy.Value = LayoutF(() => {
					...
					// Help Box
					Space(6);
					GUI.Label(GUIRect(0, 92), NAME_STRATEGY_HINT, HelpBoxStyle);
					Space(2);
				...
				LayoutF_Setting_System.Value = LayoutF(() => {
					...
					LayoutH(() => {
						...
						if (GUI.Button(GUIRect(0, 16), "Reset All Settings", EditorStyles.miniButton)) {
							bool reset = Util.Dialog("Confirm", "Reset all settings for Aseprite To Unity?", "Reset ？", "Cancel");
							if (reset) { reset = Util.Dialog("Confirm", "Reset all settings for Aseprite To Unity?", "Reset !", "Cancel"); }
							if (reset) {
								DeleteAllSaving();
		...
		private static void AboutGUI () {
			LayoutF_About.Value = LayoutF(() => {
				const int HEIGHT = 18;
				GUI.Label(GUIRect(0, HEIGHT), "Aseprite To Unity by 楠瓜Moenen.");
	...
	public class AsepriteInspector : Editor {
		...
		private void OnEnable () {
			...
			if (IsValid) {
				AsepriteToUnity_Window.MainInit();
			}
		}
		...
		public override void OnInspectorGUI () {
			...
			if (IsValid) {
				bool oldE = GUI.enabled;
				GUI.enabled = true;
				AsepriteToUnity_Window.MainGUI();
				GUI.enabled = oldE;
			}
		}
