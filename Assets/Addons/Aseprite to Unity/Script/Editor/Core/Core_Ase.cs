namespace AsepriteToUnity.Core {
	...
	public class AseCore {
		...
		public TaskResult[] CreateResults () {
			...
			if (SeparateEachLayer) {
				...
				int layerCount = Data.GetLayerCount();
		...
		private TaskResult.FrameResult GetFrameResult (AseData.CelChunk[,] cels, int frameIndex, float duration) {
					...
					if (sData != null) {
						...
						sprites.Add(new SpriteMetaData() {
							...
							pivot = UserPivotMode == UserPivotMode.SlicePivotFirst && chunk.CheckFlag(AseData.SliceChunk.SliceFlag.HasPivot) ? new Vector2(
								(sData.PivotX + 0.5f) / sData.Width,
								1f - (sData.PivotY + 0.5f) / sData.Height
							) : UserPivot,
							alignment = 9,
						});
					}
			...
			if (TrimTexture || TrimSprite) {
				...
				if (TrimSprite) {
					...
					for (int i = 0; i < sprites.Count; i++) {
						var sp = sprites[i];
						if (GetTrimOffset(pixels, width, height, out bottom, out top, out left, out right, (int)sp.rect.xMin, (int)sp.rect.yMin, (int)sp.rect.xMax - 1, (int)sp.rect.yMax - 1)) {
				...
				// Trim Texture
				if (TrimTexture && GetTrimOffset(pixels, width, height, out bottom, out top, out left, out right)) {
					// Expend for Sprite
			...
			// Root Failback Sprite (or animation sprites)
			if (sprites.Count == 0) {
				int bottom = 0, top = height - 1, left = 0, right = width - 1;
				if (TrimSprite) {
					GetTrimOffset(pixels, width, height, out bottom, out top, out left, out right);
				}
				sprites.Add(new SpriteMetaData() {
					name = AseName,
		...
		private bool GetTrimOffset (Color[] pixels, int width, int height, out int bottom, out int top, out int left, out int right) {
			return GetTrimOffset(pixels, width, height, out bottom, out top, out left, out right, 0, 0, width - 1, height - 1);
		}


		private bool GetTrimOffset (Color[] pixels, int width, int height, out int bottom, out int top, out int left, out int right, int minX, int minY, int maxX, int maxY) {
			bottom = -1;
			top = -1;
			left = -1;
			right = -1;
